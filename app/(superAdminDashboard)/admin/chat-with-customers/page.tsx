...
const INITIAL_customers: Vendor[] = [
  {
    id: "v1",
    name: "Padaria Lisboa",
...
export default function ChatWithcustomersGlass() {
  ...
  // Selected vendor
  const selected = useMemo(() => customers.find((v) => v.id === selectedId) ?? customers[0], [customers, selectedId]);
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setcustomers((p) => p.map((v) => (v.id === "v2" ? { ...v, typing: !v.typing } : v)));
    }, 7000);
  ...
  function handleSend() {
    ...
    setcustomers((prev) => prev.map((v) => (v.id === selected.id ? { ...v, messages: [...v.messages, msg], unread: 0 } : v)));

    // optimistic demo status updates
    setTimeout(() => {
      setcustomers((p) => p.map((v) => ({ ...v, messages: v.messages.map((m) => ({ ...m, status: m.status === "sent" ? "delivered" : m.status })) })));
    }, 600);
    setTimeout(() => {
      setcustomers((p) => p.map((v) => ({ ...v, messages: v.messages.map((m) => ({ ...m, status: m.status === "delivered" ? "read" : m.status })) })));
    }, 1600);
    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-gray-50 p-6">
      <div className="max-w-7xl mx-auto flex gap-6 items-start">
        ...
        <aside
          ...
          aria-label="customers (floating card)"
        >
            ...
            {filtered.map((v) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setcustomers((s) => s.map((x) => (x.id === v.id ? { ...x, pinned: !x.pinned } : x)))}
      ...
      <style jsx>{`
        ...
        @media (max-width: 900px) {
          ...
          aside[aria-label="customers (floating card)"] {
            width: 100%;
            max-width: 100%;
            min-width: auto;
            position: relative;
