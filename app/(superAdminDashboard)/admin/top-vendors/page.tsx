...
const MOCK_VENDORS: Vendor[] = [
  {
    ...
    status: "Active",
    growth: 12.4,
    description: "Premium sushi with fast prep times and consistent 5-star reviews.",
    trend: [
      { day: "Mon", value: 2400 },
      { day: "Tue", value: 3200 },
      { day: "Wed", value: 2900 },
...
export default function TopVendorsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    let list = MOCK_VENDORS.filter((v) => v.name.toLowerCase().includes(q) || v.zone.toLowerCase().includes(q));
    list = list.sort((a, b) => {
      const dir = desc ? -1 : 1;
  ...
  function exportCSV() {
    const headers = ["id", "name", "rating", "orders", "revenue", "zone", "status", "growth"];
    const rows = filtered.map((v) => [v.id, v.name, v.rating, v.orders, v.revenue, v.zone, v.status, v.growth]);
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (e.key === "k" && (e.ctrlKey || e.metaKey)) {
        // focus search
        const el = document.getElementById("vendor-search") as HTMLInputElement | null;
        el?.focus();
        e.preventDefault();
      }
    }
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-b from-white to-gray-50 text-gray-800">
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            {Array.from({ length: pageSize }).map((_, i) => (
              <div key={i} className="animate-pulse bg-white rounded-2xl p-4 shadow-sm h-40" />
            ))}
          </div>
        ) : (
          <>
                  ...
                  <article key={v.id} tabIndex={0} aria-labelledby={`vendor-${v.id}-name`} className="bg-white rounded-2xl p-4 shadow-lg hover:shadow-2xl transition-transform transform hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-offset-2" onKeyDown={(e)=>{ if(e.key === 'Enter') setSelectedVendor(v); }}>
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="text-right">
                        <div className="text-sm font-semibold">€{v.revenue.toLocaleString()}</div>
                        <div className={`text-xs ${v.growth >= 0 ? "text-green-600" : "text-red-600"}`}>{v.growth >= 0 ? `+${v.growth}%` : `${v.growth}%`}</div>
                      </div>
                    </div>
                    ...
                    <div className="mt-3">
                      <div className="h-16">
                        <ResponsiveContainer width="100%" height={60}>
                          <AreaChart data={v.trend} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                            <defs>
                            ...
                            <Area type="monotone" dataKey="value" stroke={PRIMARY} fill={`url(#g-${v.id})`} strokeWidth={2} />
                          </AreaChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
                  ...
                  <tbody className="divide-y">
                    {pageItems.map((v) => (
                      <tr key={v.id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="py-3 text-yellow-600 flex items-center gap-1"><Star size={14} /> {v.rating}</td>
                        ...
                        <td className="py-3 w-48">
                          <div className="h-12 w-48">
                            <ResponsiveContainer width="100%" height={40}>
                              <LineChart data={v.trend} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
        ...
        {selectedVendor && (
          <div role="dialog" aria-modal="true" aria-labelledby="vendor-modal-title" className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="fixed inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setSelectedVendor(null)} />
            ...
            <div className="relative max-w-3xl w-full bg-white rounded-2xl shadow-2xl p-6 z-10">
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="md:col-span-2">
                  <div className="text-sm text-gray-600">{selectedVendor.description}</div>
