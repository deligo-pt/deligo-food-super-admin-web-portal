    ...
    <div className="p-6 lg:p-10 space-y-6 overflow-x-hidden">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3">
            <ReceiptCent className="w-8 h-8 text-[#DC3173]" /> Tax Management
          </h1>
          <p className="text-sm text-gray-500 mt-1">VAT rules, reports and audit logs.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white rounded-2xl p-4 shadow-sm border flex flex-col gap-3 overflow-hidden">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-500">VAT collected (period)</p>
              <h3 className="text-2xl font-semibold">{formatCurrency(vatCollected)}</h3>
            </div>
            <div className="text-sm text-gray-500">Taxable total: {formatCurrency(taxableTotal)}</div>
          </div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border">
          <h4 className="text-sm text-gray-500">Rules</h4>
          <div className="mt-3 space-y-2">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button onClick={bulkActivate} className="px-3 py-2 rounded-md bg-green-50 text-green-800 text-sm">Activate</button>
              <button onClick={bulkDeactivate} className="px-3 py-2 rounded-md bg-yellow-50 text-yellow-800 text-sm">Deactivate</button>
              <button onClick={() => setAuditOpen(v => !v)} className="px-3 py-2 rounded-md border text-sm flex items-center gap-2"><Activity className="w-4 h-4"/> Audit</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <section className="bg-white p-4 rounded-2xl border shadow-sm">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setSelectedIds([])} className="px-3 py-1 border rounded-md">Clear</button>
          </div>
        </div>
          ...
          <table className="min-w-full text-sm">
            <thead className="text-xs text-gray-500 uppercase border-b">
              <tr>
                ...
                <th className="py-3 px-3 text-left">Tax %</th>
                <th className="py-3 px-3 text-left">Effective From</th>
              ...
              {rules.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-3 px-3 whitespace-nowrap flex items-center gap-2">
                    <button onClick={()=>openEdit(r)} className="px-3 py-1 rounded-md border inline-flex items-center gap-2"><Edit className="w-4 h-4"/> Edit</button>
                    <button onClick={()=>removeRule(r.id)} className="px-3 py-1 rounded-md border text-red-600 inline-flex items-center gap-2"><Trash className="w-4 h-4"/> Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <section className="bg-white p-4 rounded-2xl border shadow-sm">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3 mb-4">
          <h2 className="text-xl font-semibold">VAT Payout Report</h2>

          <div className="flex items-center gap-3 w-full lg:w-auto">
            <div className="flex items-center bg-white px-3 py-2 rounded-lg border w-full lg:w-72">
              <Search className="w-4 h-4 text-gray-400" />
              <input aria-label="search transactions" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search transaction, vendor, region" className="ml-2 bg-transparent outline-none text-sm w-full" />
            </div>
            ...
            <button onClick={exportCSV} className="inline-flex items-center gap-2 px-3 py-2 rounded-md border">Export</button>
          </div>
        </div>
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b text-xs text-gray-500 uppercase">
                ...
                <th className="py-3 px-3 text-left">Region</th>
                <th className="py-3 px-3 text-left">Taxable (€)</th>
                <th className="py-3 px-3 text-left">VAT (€)</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {paginated.map((t)=> (
        ...
        <div className="flex items-center justify-between p-4 border-t">
          <div className="text-sm text-gray-600">Showing <span className="font-medium">{total}</span> transactions</div>
          <div className="flex items-center gap-2">
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <aside className="w-full sm:w-96 bg-white p-6 overflow-auto border-l shadow-xl">
            ...
            <div className="space-y-2 text-sm text-gray-700">
              {auditEntries.length === 0 ? <div className="text-gray-400">No audit entries.</div> : auditEntries.map((a,i)=>(<div key={i} className="p-2 bg-gray-50 rounded-md">{a}</div>))}
            </div>
          </aside>
        </div>
      )}
      ...
      {/* toast */}
      {toast && (
        <div className="fixed right-6 bottom-6 bg-gray-900 text-white px-4 py-2 rounded-md shadow">{toast} <button className="ml-3 text-xs underline" onClick={()=>setToast(null)}>Dismiss</button></div>
      )}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-end gap-2 mt-4">
        ...
        <button onClick={handleSave} className="px-4 py-2 rounded-md bg-[#DC3173] text-white">Save</button>
      </div>
    </div>
  );
}
