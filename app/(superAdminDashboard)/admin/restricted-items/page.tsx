...
export default function RestrictedItemsPage(): JSX.Element {
  ...
  const deleteItem = (id: string) => {
    if (!confirm("Delete this restricted item? This action cannot be undone."))
      return;
  ...
  const exportCSV = () => {
    ...
    const rows = items.map((i) => ({
      ...
      category: i.category,
      reason: i.reason,
      flaggedVendors: i.flaggedVendors,
    ...
    <div className="">
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="flex items-center bg-white border rounded-full px-3 py-1 shadow-sm">
          ...
          <input
            ...
            placeholder={t("search_items_reason_category")}
            className="ml-2 outline-none w-52 sm:w-64 text-sm"
          />
        </div>
        ...
        <select
          ...
          className="border rounded-full px-4 py-2 text-sm shadow-sm"
        >
      ...
      <div className="mt-6 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div className="flex items-center gap-3">
          <button
            onClick={exportCSV}
            className="flex items-center gap-2 px-3 py-2 rounded-lg border text-sm shadow-sm"
          >
      ...
      <div className="mt-6">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((it) => (
                  <tr
                    key={it.id}
                    className={`border-b hover:bg-gray-50 transition ${compact ? "text-sm" : "text-base"}`}
                  >
                    <td className="p-4 font-medium flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-linear-to-br from-[#FFE8F0] to-[#FFDFE8] flex items-center justify-center text-[#DC3173] font-bold">
                    ...
                    <td className="p-4">
                      <span className="px-3 py-1 rounded-full bg-gray-100 text-gray-700 text-xs font-medium border">
                        {it.category}
                      </span>
                    </td>
                    ...
                    <td className="p-4 flex gap-2">
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-3 py-2 rounded-lg border text-red-600 hover:bg-red-50"
                      >
        ...
        <div className="md:hidden grid grid-cols-1 gap-4 mt-3">
          {filtered.length === 0 ? (
            <div className="p-6 bg-white rounded-2xl border text-center text-gray-500">
              {t("no_restricted_items_found")}
            </div>
          ) : (
            filtered.map((it) => (
                  ...
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-semibold">{it.name}</div>
                        <div className="text-xs text-gray-500">
                          {it.category} • {t("flagged")}: {it.flaggedVendors}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="text-xs text-gray-400 mt-2">
                      {t("updated")}: {new Date(it.updatedAt).toLocaleString()}
                    </div>
                  </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 animate-[fadeIn_200ms_ease]"
      >
        {children}
      </div>
    </div>
...
function RestrictedForm({ initial, categories, onSave, onCancel }: any) {
  ...
  const [flaggedVendors, setFlaggedVendors] = useState<number>(
    initial?.flaggedVendors ?? 0,
  );
    ...
    >
      ...
      <div>
        ...
        <input
          type="number"
          min={0}
          value={flaggedVendors}
          onChange={(e) => setFlaggedVendors(Number(e.target.value))}
          className="w-32 border rounded-xl px-3 py-2 mt-1"
        />
      </div>
      ...
      <div className="flex justify-end gap-2">
        ...
        <button
          ...
          style={{ background: "#DC3173" }}
        >
          {initial ? t("save_changes") : t("add_item")}
        </button>
      </div>
    </form>
  );
