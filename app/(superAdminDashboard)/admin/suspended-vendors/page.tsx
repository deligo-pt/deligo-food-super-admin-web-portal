...
export default function SuspendedVendorsPage() {
  ...
  const [filterReason, setFilterReason] = useState<'all' | 'docs' | 'fraud' | 'violation'>('all');
  ...
  const filtered = vendors
    .filter((v) =>
      [v.name, v.owner, v.city, v.cuisine, v.reason].join(' ').toLowerCase().includes(query.toLowerCase())
    )
    .filter((v) => {
      ...
      <div className="max-w-full mx-auto">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Manage and review vendors suspended due to policy violations, fraud or document issues.</p>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <Button size="sm" variant="ghost" onClick={() => { setQuery(''); setFilterReason('all'); }}>
              Reset
            </Button>
          </div>
        </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <Card className="p-4 flex items-center gap-3 shadow-sm border-l-4 border-red-500">
                ...
                <div>
                  ...
                  <div className="font-semibold text-slate-900">{vendors.length}</div>
                </div>
              </Card>
            </div>
          ...
          <Card className="p-0 overflow-hidden shadow-md">
            <div className="p-4 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-medium text-lg flex items-center gap-2"><Ban className="w-4 h-4 text-red-600" /> Suspended Vendor List</h2>
                ...
                <Table>
                  ...
                  <TableBody>
                    {loading ? (
                      <TableRow>
                        <TableCell colSpan={6}>
                          <div className="py-12 text-center text-slate-500">Loading suspended vendors...</div>
                        </TableCell>
                      </TableRow>
                    ) : paginated.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={6}>
                          <div className="py-12 text-center text-slate-500">No suspended vendors found.</div>
                        </TableCell>
                      </TableRow>
                    ) : (
                      paginated.map((v) => (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <div className="font-semibold flex items-center gap-2">
                                  <Store className="w-4 h-4 text-slate-700" /> {v.name}
                                </div>
                          ...
                          <TableCell><span className="flex items-center gap-2"><MapPin className="w-4 h-4" /> {v.city}</span></TableCell>
                          ...
                          <TableCell>{new Date(v.suspendedAt).toLocaleDateString('en-GB')}</TableCell>

                          <TableCell>
                            <div className="flex items-center gap-2">
                              <FileText className="w-4 h-4" />
                              <span>{v.documentsFlagged ? 'Flagged' : 'Clear'}</span>
                            </div>
                          </TableCell>
                          ...
                          <TableCell className="text-right pr-6">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <Button size="sm" style={{ background: DELIGO }} onClick={() => reinstateVendor(v.id)}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="col-span-1">
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    <p className="text-sm text-slate-500 flex items-center gap-2"><MapPin className="w-4 h-4" /> {selected.city}</p>
                  </div>
                </div>

                <div className="col-span-2">
                  <h4 className="text-sm font-medium flex items-center gap-2"><Info className="w-4 h-4" /> Suspension Details</h4>
                  <div className="mt-2 space-y-2 text-sm">
                    <div><strong>Owner:</strong> {selected.owner}</div>
                    <div><strong>Phone:</strong> {selected.phone}</div>
                    <div><strong>Suspended On:</strong> {new Date(selected.suspendedAt).toLocaleString()}</div>
                    <div className="flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-red-600" /> {selected.reason}</div>
                  </div>

                  <Separator className="my-4" />

                  <h4 className="text-sm font-medium flex items-center gap-2"><FileText className="w-4 h-4" /> Zones</h4>
                  ...
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <Button style={{ background: DELIGO }} onClick={() => { reinstateVendor(selected.id); setSelected(null); }}>
                      <Undo2 className="w-4 h-4 mr-2" /> Reinstate
                    </Button>
                  </div>
                </div>
...
function mockSuspendedVendors(): SuspendedVendor[] {
  ...
  return Array.from({ length: 20 }).map((_, i) => ({
    id: `S-VND-${3000 + i}`,
    name: ['Casa Lisboa', 'Urban Grill', 'Pasta Nova', 'Veggi Porto', 'Sushi Wave'][i % 5],
    ...
    zones: ['Lisbon Central', 'Porto Centro', 'Coimbra Oeste'].slice(0, (i % 3) + 1),
  }));
}
