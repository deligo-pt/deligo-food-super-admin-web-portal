  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
        <div>
          <h1 className="text-3xl md:text-4xl font-extrabold">Coupon Analytics</h1>
          <p className="text-gray-600 text-sm">Performance insights for all coupon campaigns across Portugal.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-8">
        <KpiCard title="Total Coupons Used" value={totalUsed} icon={<TicketPercent size={28} />} color="from-[#DC3173] to-pink-500" />
        <KpiCard title="Total Coupons Issued" value={totalIssued} icon={<Calendar size={28} />} color="from-blue-500 to-blue-400" />
        <KpiCard title="Success Rate" value={`${successRate}%`} icon={<Percent size={28} />} color="from-green-500 to-emerald-400" />
      </div>

      {/* Top Performing Coupons */}
      <div className="mt-10 bg-white border rounded-3xl p-6 shadow">
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
          <BarChart3 size={20} className="text-[#DC3173]" /> Top Performing Coupons
        </h2>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-100 text-gray-600 border-b">
              <tr>
                ...
                <th className="p-4 text-left">Issued</th>
                <th className="p-4 text-left">Usage Rate</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((c) => (
        ...
        <div className="md:hidden grid grid-cols-1 gap-4">
          {filtered.map((c) => (
            <div key={c.id} className="p-4 bg-white border rounded-xl shadow-sm">
              ...
              <p className="text-sm text-gray-500 mt-1">Vendor: {c.vendor}</p>
              <p className="mt-2 text-sm">Used: <span className="font-semibold">{c.used}</span></p>
              <p className="text-sm">Issued: {c.issued}</p>
              <p className="text-sm mt-1 font-semibold text-[#DC3173]">Usage Rate: {((c.used / c.issued) * 100).toFixed(1)}%</p>
            </div>
          ))}
        </div>
      </div>
...
function KpiCard({ title, value, icon, color }: any) {
  return (
    <div className={`rounded-2xl p-6 text-white shadow bg-gradient-to-br ${color} flex items-center gap-4`}>
