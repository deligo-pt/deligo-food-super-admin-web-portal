  ...
  return (
    <div className="">
      ...
      <TitleHeader
        ...
        subtitle={t("performance_insights_coupon_campaigns")}
      />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-8">
        <KpiCard
          title={t("total_coupons_used")}
          value={totalUsed}
          icon={<TicketPercent size={28} />}
          color="from-[#DC3173] to-pink-500"
        />

        <KpiCard
          title={t("total_coupons_issued")}
          value={totalIssued}
      ...
      <div className="mt-10 bg-white border rounded-3xl p-6 shadow">
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
          <BarChart3 size={20} className="text-[#DC3173]" />{" "}
          {t("top_performing_coupons")}
        </h2>
            ...
            <tbody>
              {filtered.map((c) => (
                <tr key={c.id} className="border-b hover:bg-gray-50 transition">
                  ...
                  <td className="p-4 font-semibold text-[#DC3173]">
                    {((c.used / c.issued) * 100).toFixed(1)}%
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          {filtered.map((c) => (
            ...
            >
              ...
              <p className="mt-2 text-sm">
                {t("used")}: <span className="font-semibold">{c.used}</span>
              </p>
              <p className="text-sm">
                {t("issued")}: {c.issued}
              </p>
              <p className="text-sm mt-1 font-semibold text-[#DC3173]">
                {t("usage_rate")}: {((c.used / c.issued) * 100).toFixed(1)}%
              </p>
            </div>
          ))}
        </div>
...
function KpiCard({ title, value, icon, color }: any) {
  return (
    <div
      className={`rounded-2xl p-6 text-white shadow bg-linear-to-br ${color} flex items-center gap-4`}
    >
