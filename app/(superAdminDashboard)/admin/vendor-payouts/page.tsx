...
export default function VendorPayoutsPage() {
  ...
  const filtered = payouts.filter((p) =>
    [p.vendorName, p.city, p.payoutMethod, p.status, p.id].join(' ').toLowerCase().includes(query.toLowerCase())
  );
  ...
  function exportCSV() {
    const rows = [
      ['Payout ID', 'Vendor ID', 'Vendor Name', 'Amount (€)', 'Method', 'City', 'Date', 'Status', 'IBAN', 'Account Holder'],
      ...filtered.map((p) => [
        p.id,
        p.vendorId,
        p.vendorName,
    ...
    a.href = url;
    a.download = `vendor_payouts_${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
                    ...
                    <motion.tr key={p.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -4 }} className="bg-white">
                      <TableCell className="pl-6">
                        <div className="flex items-center gap-3">
                          <Avatar>
                            <AvatarImage src={`/images/vendors/${p.vendorId}.jpg`} alt={p.vendorName} />
                            <AvatarFallback>{p.vendorName[0]}</AvatarFallback>
                          </Avatar>
                          <div>
                            <div className="font-semibold">{p.vendorName}</div>
                            <div className="text-xs text-slate-400">{p.vendorId}</div>
                          </div>
                        </div>
                      </TableCell>
            ...
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.22 }}>
              <div className="flex items-center gap-4 mb-4">
                <Avatar className="w-16 h-16">
                  <AvatarImage src={`/images/vendors/${selected.vendorId}.jpg`} alt={selected.vendorName} />
                  <AvatarFallback>{selected.vendorName[0]}</AvatarFallback>
                </Avatar>

                <div>
                  <h3 className="text-xl font-semibold">{selected.vendorName}</h3>
