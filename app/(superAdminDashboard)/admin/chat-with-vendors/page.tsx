...
export default function ChatWithVendorsGlass() {
  ...
  // Selected vendor
  const selected = useMemo(() => vendors.find((v) => v.id === selectedId) ?? vendors[0], [vendors, selectedId]);
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setVendors((p) => p.map((v) => (v.id === "v2" ? { ...v, typing: !v.typing } : v)));
    }, 7000);
  ...
  function handleSend() {
    ...
    setVendors((prev) => prev.map((v) => (v.id === selected.id ? { ...v, messages: [...v.messages, msg], unread: 0 } : v)));

    // optimistic demo status updates
    setTimeout(() => {
      setVendors((p) => p.map((v) => ({ ...v, messages: v.messages.map((m) => ({ ...m, status: m.status === "sent" ? "delivered" : m.status })) })));
    }, 600);
    setTimeout(() => {
      setVendors((p) => p.map((v) => ({ ...v, messages: v.messages.map((m) => ({ ...m, status: m.status === "delivered" ? "read" : m.status })) })));
    }, 1600);
            ...
            {filtered.map((v) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setVendors((s) => s.map((x) => (x.id === v.id ? { ...x, pinned: !x.pinned } : x)))}
