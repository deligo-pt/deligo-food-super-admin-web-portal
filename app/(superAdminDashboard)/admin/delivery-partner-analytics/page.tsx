...
// ---------------- Types ----------------
type MonthPoint = { month: string; deliveries: number; revenue: number; growthPct?: number };
type Partner = {
  id: string;
  name: string;
  city: string;
...
export default function DeliveryPartnerAnalyticsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return partners.filter((p) => (q ? `${p.name} ${p.city} ${p.id}`.toLowerCase().includes(q) : true));
  }, [partners, query]);
  ...
  function heatColor(value: number) {
    ...
    const r1 = 220, g1 = 49, b1 = 115; // DELIGO hex DC3173
            ...
            <div className="h-56 mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={aggregatedMonths}>
                  ...
                  <XAxis dataKey="month" />
                  <YAxis />
                  <Tooltip formatter={(val: any, name: string) => (name === 'revenue' ? `€ ${Number(val).toLocaleString()}` : val)} />
...
function KpiCard({ title, value, icon, accent = DELIGO }: { title: string; value: string; icon?: React.ReactNode; accent?: string }) {
  return (
    ...
    <div>
      <div className="grid grid-cols-6 gap-2">
        {aggregated.map((val, i) => {
          ...
          return (
            <button key={i} onClick={() => onCellClick(i, val)} className="p-3 rounded text-center" style={{ background: color }}>
              <div className="font-semibold">{i}:00</div>
      ...
      <div className="mt-2 text-xs text-slate-500">Click a cell to drill into partners for that hour.</div>
    </div>
  );
}
