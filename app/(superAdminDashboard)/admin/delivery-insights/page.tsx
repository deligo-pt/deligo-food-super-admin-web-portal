    ...
    <div className="min-h-screen p-6 bg-gray-50 text-gray-800">
      <div className="max-w-7xl mx-auto">
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold">{t("delivery_insights")}</h1>
            <p className="text-sm text-gray-600 mt-1">{t("driver_performance_sla_heat_zones")}</p>
          </div>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm text-gray-500">{t("total_deliveries")}</h3>
            ...
            <p className="text-xs text-gray-500 mt-3">{range} • {t("portugal")}</p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            ...
            <p className="text-xs text-green-600 mt-3">{t("good_monitor_peak_hours")}</p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-semibold mt-1">{summary.avgDeliveryMin} {t("min")}</p>
              </div>
              <div className="p-3 rounded-lg" style={{ background: "#DC317311" }}>
                <Clock size={28} color="#DC3173" />
              </div>
            </div>
            <p className="text-xs text-gray-500 mt-3">{t("imporve_pickup_times_lower")}</p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            ...
            <p className="text-xs text-red-600 mt-3">{(summary.cancellations / Math.max(1, summary.totalDeliveries) * 100).toFixed(1)}% {t("of_deliveries")}</p>
          </div>
        </section>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">{t("delivery_volume_hour_use_balance_driver")}</p>
              </div>

              <div className="text-sm text-gray-600">{t("heat_view_available")}</div>
            </div>
            ...
            <div className="mt-4 flex gap-3 flex-wrap">
              <div className="text-xs bg-gray-50 px-3 py-1 rounded-full">{t("drivers")}: active</div>
              <div className="text-xs bg-gray-50 px-3 py-1 rounded-full">{t('zones')}: busy</div>
              <div className="text-xs bg-gray-50 px-3 py-1 rounded-full">{t("sla")}: 84%</div>
            </div>
          </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <div key={d.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">{t("avg_pickup")} {d.avgPickupMin} {t("min")}</div>
                    </div>
                  </div>
                  <div className="text-sm text-gray-600">{d.completed} {t("completed")} • {d.late} {t("late")}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm">
            <h3 className="text-lg font-semibold mb-3">{t("actionable_alerts")}</h3>
        ...
        <section className="bg-white rounded-2xl p-5 shadow-sm">
          <h3 className="text-lg font-semibold mb-3">{t("delivery_log_sample")}</h3>
