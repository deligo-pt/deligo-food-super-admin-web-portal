...
export default function TransactionHistoryPage(): JSX.Element {
  ...
  const [preset, setPreset] =
    useState<(typeof datePresets)[number]>("Last 30 days");
  ...
  const [statusFilter, setStatusFilter] = useState<"all" | TxRow["status"]>(
    "all",
  );
  ...
  const inflow = filtered
    .filter((r) => r.amount > 0 && r.type !== "Payout")
    .reduce((s, r) => s + r.amount, 0);
  const outflow = filtered
    .filter((r) => r.type === "Payout")
    .reduce((s, r) => s + r.amount, 0);
  const currentBalance = paginated.length
    ? paginated[0].balance
    : (filtered[0]?.balance ?? 0);
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {t("full_ledger_platform_transactions_payouts")}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border bg-white shadow-sm"
          >
          ...
          <button
            onClick={exportCSV}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#DC3173] text-white shadow hover:opacity-95"
          >
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto flex-wrap">
          <div className="flex items-center bg-white px-3 py-2 rounded-lg border w-full lg:w-64">
            ...
            <input
              ...
              placeholder={t("search_tx_id_partner_type")}
            ...
            <tbody className="divide-y">
              {paginated.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td
                    className={`py-3 px-3 whitespace-nowrap font-semibold ${r.type === "Payout" ? "text-red-600" : "text-green-700"}`}
                  >
                    {formatCurrency(r.amount)}
                  </td>
                  <td className="py-3 px-3 whitespace-nowrap text-[#DC3173] font-medium">
                    {formatCurrency(r.balance)}
                  </td>
                  <td className="py-3 px-3 whitespace-nowrap">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${r.status === "Success" ? "bg-green-100 text-green-800" : r.status === "Processing" ? "bg-yellow-100 text-yellow-800" : "bg-red-100 text-red-800"}`}
                    >
                      {r.status}
                    </span>
                  </td>
              ...
              {paginated.length === 0 && (
                <tr>
                  <td colSpan={7} className="text-center py-8 text-gray-400">
                    {t("no_transactions_match_filters")}
                  </td>
                </tr>
              )}
            </tbody>
