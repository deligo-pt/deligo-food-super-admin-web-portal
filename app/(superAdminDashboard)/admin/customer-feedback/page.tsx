...
export default function CustomerFeedbackPremium() {
  ...
  const cities = useMemo(
    () =>
      Array.from(new Set(data.map((d) => d.city).filter(Boolean) as string[])),
    [data],
  );
  ...
  const filtered = useMemo(() => {
    return data.filter((f) => {
      ...
      if (
        q &&
        ![f.name, f.email, f.comment, f.tags?.join(" ") || ""]
          .join(" ")
          .toLowerCase()
          .includes(q)
      )
  ...
  const stats = useMemo(() => {
    ...
    const positive = total
      ? (data.filter((d) => d.sentiment === "positive").length / total) * 100
      : 0;
    return {
      total,
  ...
  return (
    <div className="min-h-screen p-6 bg-slate-50">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-6">
        <Card className="p-4">
          ...
          <h3 className="text-2xl font-bold">
            {stats.avg} <span className="text-sm text-slate-500">/ 5</span>
          </h3>
        </Card>
        ...
        <Card className="p-4">
          ...
          <div className="mt-2">
            <Sparkline data={data.slice(0, 12).map((d) => d.rating)} />
          </div>
        </Card>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <Input
          placeholder={t("search_comments_names_tags")}
          value={query}
        ...
        <Select
          ...
          onValueChange={(v) => setCityFilter(v === "all" ? "all" : (v as any))}
        >
            ...
            <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="col-span-2">
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex gap-3 flex-wrap">
                      {selected.attachments.map((a, i) => (
                        <div
                          key={i}
                          className="w-32 h-20 bg-slate-100 rounded-md flex items-center justify-center text-xs"
                        >
                          {a.name}
                        </div>
                      ))}
                ...
                <div className="mt-6">
                  ...
                  <ReplyForm
                    feedback={selected}
                    onReply={(text) => replyToFeedback(selected.id, text)}
                  />
                </div>
              </div>
      ...
      >
        <DialogContent className="max-w-md">
          ...
          <p className="text-sm text-slate-600 mb-4">
            {t("are_you_sure_want_delete_feedback")}
          </p>
          <DialogFooter>
  ...
  return (
    <div>
      <textarea
        ...
        className="w-full p-3 rounded-md border resize-y min-h-[100px]"
      />
...
function mockFeedback(): Feedback[] {
  ...
  return Array.from({ length: 28 }).map((_, i) => {
    ...
    return {
      ...
      attachments:
        i % 7 === 0 ? [{ name: "photo.jpg", url: SAMPLE_IMAGE }] : [],
      replied: i % 6 === 0,
    } as Feedback;
  });
}
