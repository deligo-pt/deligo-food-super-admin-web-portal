...
import { AlertTriangle, Bell, CheckCircle, Send, Users } from "lucide-react";
...
const TEMPLATES = [
  {
    ...
    body: "Heads-up: We will have short maintenance at 02:00 — deliveries may be delayed.",
  },
  {
    id: "t2",
    title: "Surge Pay",
    body: "Surge pay active in Lisbon Central for the next 2 hours. Accept any orders to earn more!",
  },
  {
    id: "t3",
    title: "Document Reminder",
    body: "Please upload your ID and business license to avoid suspension.",
  },
];
...
export default function SendNotificationFleetPage() {
  ...
  const charWarning =
    charCount > 1024 ? "Message too long ( > 1024 chars )" : null;
  ...
  async function confirmSend() {
    ...
    if (scheduleMode === "later" && !scheduledAt) {
      setToast({
        kind: "error",
        message: "Please select date & time for scheduled send",
      });
      return;
    }
    ...
    try {
      ...
      setToast({
        kind: "success",
        message: `Notification queued to ${audienceCount} recipients`,
      });
      // reset form (partial)
      setTitle("");
      setMessage("");
      ...
      setToast({
        kind: "error",
        message: (err as Error).message || "Failed to send",
      });
    } finally {
      setSending(false);
    }
  ...
  return (
    <div className="min-h-screen p-6 bg-slate-50">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                {t("compose_message_send_to_fleet")}
              </p>
            </div>

            <div className="text-right">
              <div className="text-xs text-slate-500">{t("recipients_lg")}</div>
              <div className="text-xl font-bold" style={{ color: DELIGO }}>
                {audienceCount}
              </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder={t("short_title_max_100_chars")}
                maxLength={100}
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder={t("write_notification_message")}
                aria-label="notification-body"
              />
              <div className="flex items-center justify-between text-xs text-slate-500 mt-1">
                <div>
                  {charWarning ? (
                    <span className="text-red-600">{charWarning}</span>
                  ) : (
                    <span>
            ...
            <div>
              ...
              <div className="mt-2 flex items-center gap-3">
                <Select
                  value={selectedGroup}
                  onValueChange={(v) => setSelectedGroup(v as AudienceGroup)}
                >
                ...
                {selectedGroup === "zone" && (
                  <div className="flex gap-2 flex-wrap">
                    {mockZones.map((z) => (
                      <Badge
                        key={z}
                        className={`px-3 py-1 cursor-pointer ${selectedZones.includes(z) ? "bg-black text-white" : "bg-slate-100"}`}
                ...
                {selectedGroup === "managers" && (
                  <div className="flex gap-2 flex-wrap">
                    {mockManagers.map((m) => (
                      <Badge
                        key={m}
                        className={`px-3 py-1 cursor-pointer ${selectedManagers.includes(m) ? "bg-black text-white" : "bg-slate-100"}`}
            ...
            <div className="flex items-center justify-between mt-4">
              <div className="text-sm text-slate-500">
                {t("do_quick_preview_confirm_before")}
              </div>
        ...
        <Card className="p-6 shadow-sm rounded-2xl">
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="font-medium">
                {title || (
                  <span className="text-slate-400">{t("untitled")}</span>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="text-sm text-slate-700 whitespace-pre-line">
                {message || (
                  <span className="text-slate-400">{t("no_message_yet")}</span>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-500">
                  {t("how_notification_look_on_devices")}
                </div>
              </div>
            </div>
          </DialogHeader>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <div className="text-sm text-slate-500 mt-1">
              {t("your_re_sending_to")} <strong>{audienceCount}</strong>{" "}
              {t("recipients")}.
            </div>
          </DialogHeader>
          ...
          <div className="mt-4 space-y-3">
            ...
            <div>
              <div className="text-xs text-slate-500">{t("message")}</div>
              <div className="text-sm text-slate-700 whitespace-pre-line max-h-40 overflow-auto">
                {message}
              </div>
            </div>
            ...
            <div className="pt-2 text-sm text-slate-500">
              {t("this_action_irreversible_queued_batch")}
            </div>
          </div>
      ...
      {toast && (
        <div
          className={`fixed bottom-6 right-6 px-4 py-3 rounded-xl shadow-lg flex items-center gap-3 ${toast.kind === "success" ? "bg-emerald-600 text-white" : "bg-rose-600 text-white"}`}
        >
          {toast.kind === "success" ? (
