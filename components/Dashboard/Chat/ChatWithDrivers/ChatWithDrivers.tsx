...
import SelectDriverModal from "@/components/Dashboard/Chat/ChatWithDrivers/SelectDriverModal";
...
export default function ChatWithDrivers({
  conversationsData,
  accessToken,
  decoded,
}: IProps) {
  ...
  const selected = useMemo(() => {
    const participant = conversations
      ...
      ?.participants?.find((p) => p.role === "DELIVERY_PARTNER");
    return participant;
  ...
  const getNewConversation = async (message: {
    ...
    if (message?.message?.senderRole === "DELIVERY_PARTNER") {
  ...
  const selectDriver = async (driver: TDeliveryPartner) => {
    ...
    const existingConversation = conversations.find((c) =>
      c.participants?.some(
        (p) => p.userId === driver.userId && p.role === "DELIVERY_PARTNER",
      ),
    );
    if (existingConversation && existingConversation.type === "DRIVER_CHAT") {
    ...
    <div className="h-full">
      <div className="flex gap-6 items-start h-full">
              ...
              >
                ...
                >
                  <div className="w-12 h-12 rounded-full bg-[#DC3173]/12 text-[#DC3173] font-semibold overflow-hidden flex items-center justify-center">
                    {c.participants
                      ?.find((p) => p.role === "DELIVERY_PARTNER")
                      ?.name?.trim()?.[0] ?? `V${i + 1}`}
                  </div>
        ...
        <SelectDriverModal
          ...
          onClick={(driver: TDeliveryPartner) => selectDriver(driver)}
