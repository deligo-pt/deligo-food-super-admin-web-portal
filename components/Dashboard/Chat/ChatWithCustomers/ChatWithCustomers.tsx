...
import SelectCustomerModal from "@/components/Dashboard/Chat/ChatWithCustomers/SelectCustomerModal";
...
export default function ChatWithCustomers({
  conversationsData,
  accessToken,
  decoded,
}: IProps) {
  ...
  const getNewConversation = async (message: {
    ...
    if (message?.message?.senderRole === "CUSTOMER") {
  ...
  const selectCustomer = async (customer: TCustomer) => {
    ...
    const existingConversation = conversations.find((c) =>
      c.participants?.some(
        (p) => p.userId === customer.userId && p.role === "CUSTOMER",
      ),
    );

    if (existingConversation && existingConversation.type === "CUSTOMER_CHAT") {
    ...
    <div className="h-full">
      <div className="flex gap-6 items-start h-full">
        ...
        <SelectCustomerModal
          ...
          onClick={(customer: TCustomer) => selectCustomer(customer)}
