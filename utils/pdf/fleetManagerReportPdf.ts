...
export const generateFleetManagerReportPDF = (
    ...
    autoTable(doc, {
        ...
        body: fleetManagers.map((f) => [
            `${f.name?.firstName ?? ""} ${f.name?.lastName ?? ""}`.trim() || "N/A",
            f.operationalData?.totalDrivers ?? 0,
            f.operationalData?.totalDeliveries ?? 0,
            f.operationalData?.rating?.average ?? 0,
            f.createdAt ? format(new Date(f.createdAt), "yyyy-MM-dd") : "N/A",
            f.status,
        ]),
        styles: {
            fontSize: 9,
